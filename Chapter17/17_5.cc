...
// using a pair that holds an index and a pair of iterators instead

using matches = std::pair<std::vector<Sales_data>::size_type, std::pair<std::vector<Sales_data>::const_iterator,
...
std::vector<matches> findBook(const std::vector<std::vector<Sales_data>> &trans, const std::string &book) {
	...
	for(auto store_it = trans.cbegin(); store_it != trans.cend(); ++store_it) {
		...
		if(found.first != found.second) {
			res.push_back(std::make_pair(store_it - trans.cbegin(), std::make_pair(found.first, found.second)));
		}
	}
	return res;
}
...
void reportResults(std::istream &is, std::ostream &os, const std::vector<std::vector<Sales_data>> &trans) {
	...
	while(is >> s) {
		...
		for(const auto &elem : find_book) {
			os << "For book " << s << ", the store with index " << elem.first 
			   << " has following transaction sum: \n" << std::accumulate(elem.second.first, elem.second.second, Sales_data(s))
			   << std::endl; 
		}
	}
}
