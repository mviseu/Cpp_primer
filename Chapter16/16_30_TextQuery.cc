...
TextQuery::TextQuery(ifstream &input) {
	for(string line; getline(input, line); ) {
		...
		while(input_line >> word) {
			...
			wm.insert(make_pair(word, SharedPtr<set<size_t>>(new set<size_t>(), DebugDelete())));
...
QueryResult TextQuery::query(const string &word) const {
	...
	if(ret == wm.end()) {
		return QueryResult(word, file, SharedPtr<set<size_t>>(new set<size_t>(), DebugDelete()));
	} else {
