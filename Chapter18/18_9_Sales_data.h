...
struct mismatch_isbn : public std::logic_error {
  mismatch_isbn(const std::string &mess, const std::string &lhs, const std::string &rhs) 
  : std::logic_error(mess), left(lhs), right(rhs) {}
  explicit mismatch_isbn(const std::string &mess) : mismatch_isbn(mess, "", "") {}
...
struct out_of_stock : public std::runtime_error {
  explicit out_of_stock(const std::string &mess) : std::runtime_error(mess) {}
};
...
struct Sales_data {
  ...
  Sales_data &operator+=(const Sales_data &rhs) {
    if(isbn() != rhs.isbn()) {
      throw mismatch_isbn("isbn does not match", isbn(), rhs.isbn());
    }
